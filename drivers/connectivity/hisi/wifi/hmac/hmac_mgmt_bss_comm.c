                ...
                oal_uint32                  frame_body_len)
{
    ...
    if (DMAC_BA_INIT == pst_tid->st_ba_tx_info.en_ba_status)
    {
        ...
        st_action_args.ul_arg3     = MAC_UNSPEC_QOS_REASON;         /* DELBA??????????reason */
        st_action_args.puc_arg5    = pst_hmac_user->st_user_base_info.auc_user_mac_addr;    /* DELBA?????????????? */
                                                     ...
                                                     oal_uint16 us_init_trans_id)
{
    ...
    pst_tx_ctl->us_mpdu_len     = us_sa_query_len;               /* dmac??????????mpdu???? */
...
oal_void  hmac_mgmt_send_deauth_frame(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_da, oal_uint16 us_err_code, oal_bool_enum_uint8 en_is_protected)
{
    ...
    pst_tx_ctl->us_mpdu_len   = us_deauth_len;                                      /* dmac??????????mpdu???? */
    pst_tx_ctl->us_tx_user_idx              = 0xffff;  /* ????????????????user?????? */
...
oal_uint32 hmac_wpas_mgmt_tx(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
   ...
   // mac_cfg_add_user_param_stru     st_add_user;//TBD:P2P DKY BUG ,????????????????????????
   // oal_uint8                      *puc_da;
    ...
    pst_tx_ctl->us_mpdu_len       = pst_mgmt_tx->us_len;                                      /* dmac??????????mpdu???? */
    //pst_tx_ctl->us_tx_user_idx    = st_add_user.us_user_idx;  /* ????????????????user?????? */
