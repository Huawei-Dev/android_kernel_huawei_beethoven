...
typedef struct
{
    oal_uint8  uc_fbt_mode;              /* ????????????????????????0????????????1??????????AC???????? */
    oal_uint8  uc_disabled_user_cnt;     /* ?????????????????????????????? */
    oal_uint8  auc_rev[2];
    hmac_fbt_disable_user_info_stru ast_fbt_disable_connect_user_list[HMAC_FBT_MAX_USER_NUM];  /* ????32??????????????????????32????????????????????3?????? */
...
typedef struct hmac_vap_tag
{
    ...
    oal_uint16                      us_user_nums_max;                           /* VAP?????????????????????? */
    oal_uint8                       uc_classify_tid;                            /* ????????vap?????????????????? */
    wlan_auth_alg_enum_uint8        en_auth_mode;                               /* ???????? */
    ...
    oal_uint8                       uc_80211i_mode;                             /* ????????????????WPA????WPA2, bit0 = 1,WPA; bit1 = 1, RSN */
    oal_uint8                       uc_ba_dialog_token;                         /* BA???????????????? */
#ifdef _PRE_WLAN_FEATURE_PROXYSTA
    hmac_psta_stru                  st_psta;
#endif
#ifdef _PRE_WLAN_FEATURE_CUSTOM_SECURITY
    mac_blacklist_info_stru         st_blacklist_info;                          /* ?????????? */
...
#ifdef _PRE_WLAN_FEATURE_HS20
    oal_uint8                       uc_is_interworking;                         /* ??????STA??????AP????????interworking???? */
    ...
    frw_timeout_stru                  st_40M_recovery_timer;                    /* 40M?????????? */
    wlan_channel_bandwidth_enum_uint8 en_40M_bandwidth;                         /* ????ap????????20M?????????? */
    ...
    oal_uint8                         uc_rx_ba_session_num;                   /* ??vap????rx BA?????????? */
    oal_uint8                         uc_tx_ba_session_num;                   /* ??vap????tx BA?????????? */
...
#ifdef _PRE_WLAN_FEATURE_HILINK
    hmac_fbt_mgmt_stru               st_fbt_mgmt;                               /* ????fbt?????????????????????? */
#endif
