...
typedef enum
{
    /* ????80211-2012 ???? Table 8-99 Cipher suite selectors ???? */
    ...
    WLAN_80211_CIPHER_SUITE_WAPI         = 10,      /* ??????????????11i???? */
...
typedef enum
{
    ...
    WLAN_BAND_WIDTH_40PLUS              = 1,    /* ??20????+1 */
    WLAN_BAND_WIDTH_40MINUS             = 2,    /* ??20????-1 */
    WLAN_BAND_WIDTH_80PLUSPLUS          = 3,    /* ??20????+1, ??40????+1 */
    WLAN_BAND_WIDTH_80PLUSMINUS         = 4,    /* ??20????+1, ??40????-1 */
    WLAN_BAND_WIDTH_80MINUSPLUS         = 5,    /* ??20????-1, ??40????+1 */
    WLAN_BAND_WIDTH_80MINUSMINUS        = 6,    /* ??20????-1, ??40????-1 */
...
typedef enum
{
    WLAN_SCAN_MODE_FOREGROUND,          /* ????????????AP/STA(??????????????????) */
    WLAN_SCAN_MODE_BACKGROUND_STA,      /* STA???????? */
    WLAN_SCAN_MODE_BACKGROUND_AP,       /* AP????????(??????) */
    WLAN_SCAN_MODE_BACKGROUND_OBSS,     /* 20/40MHz??????obss???? */
    WLAN_SCAN_MODE_BACKGROUND_CCA,
    WLAN_SCAN_MODE_BACKGROUND_PNO,      /* PNO???????? */
